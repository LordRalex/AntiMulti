...
public class FileManager {
    ...
    public static void openConnection()
    {
        testConnection();
        if(useSQL) {
            AMLogger.warning("The SQL Database for AntiMulti is not configured yet");
            useSQL = false;
            //openConnection();
            //SQL.connect();
    ...
    public static String[] getNames(String ip, String name) throws SQLDataException, IOException {
        if(useSQL)
            return SQL.getNames(ip, name);
        else
        {
            if(useSQL)
            return SQL.getIPs(name);
    ...
    public static String[] getIPs(String name, String ip) throws SQLDataException, IOException {
        if(useSQL)
            return SQL.getIPs(name, ip);
        else
        {
            if(useSQL)
            return SQL.getIPs(name);
